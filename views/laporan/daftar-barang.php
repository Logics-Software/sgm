...
if (!function_exists('getSortIconLaporan')) {
    function getSortIconLaporan($column, $currentSortBy, $currentSortOrder) {
        $config = require __DIR__ . '/../../config/app.php';
        $baseUrl = rtrim($config['base_url'], '/');
        if (empty($baseUrl) || $baseUrl === 'http://' || $baseUrl === 'https://') {
            $baseUrl = '/';
        ...
        <div class="card">
            <div class="card-header bg-white">
                <div class="d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">Laporan Daftar Barang</h4>
            ...
            <div class="card-body">
                <form method="GET" action="/laporan/daftar-barang" class="mb-3">
                    <div class="row g-2 align-items-end search-filter-card">
                        ...
                        <div class="col-6 col-md-1">
                            <a href="/laporan/daftar-barang" class="btn btn-outline-secondary">Reset</a>
                        </div>
                    </div>
                </form>
                    ...
                    <table class="table table-striped table-hover align-middle">
                        <thead class="table-light">
                            <tr>
                                <th>No</th>
                                <th>
                                    <a href="<?= getSortUrlLaporan('kodebarang', $sortBy ?? 'namabarang', $sortOrder ?? 'ASC', $search ?? '', $perPage ?? 50, $kodepabrik ?? '', $kodegolongan ?? '', $kondisiStok ?? 'semua') ?>" class="text-decoration-none text-dark d-inline-flex align-items-center gap-1">
                                        Kode Barang
                                        <?= getSortIconLaporan('kodebarang', $sortBy ?? 'namabarang', $sortOrder ?? 'ASC') ?>
                                    </a>
                                </th>
                                <th>
                                    <a href="<?= getSortUrlLaporan('namabarang', $sortBy ?? 'namabarang', $sortOrder ?? 'ASC', $search ?? '', $perPage ?? 50, $kodepabrik ?? '', $kodegolongan ?? '', $kondisiStok ?? 'semua') ?>" class="text-decoration-none text-dark d-inline-flex align-items-center gap-1">
                                        Nama Barang
                                        <?= getSortIconLaporan('namabarang', $sortBy ?? 'namabarang', $sortOrder ?? 'ASC') ?>
                                    </a>
                                </th>
                                <th>Satuan</th>
                                <th>
                                    <a href="<?= getSortUrlLaporan('pabrik', $sortBy ?? 'namabarang', $sortOrder ?? 'ASC', $search ?? '', $perPage ?? 50, $kodepabrik ?? '', $kodegolongan ?? '', $kondisiStok ?? 'semua') ?>" class="text-decoration-none text-dark d-inline-flex align-items-center gap-1">
                                        Pabrik
                                        <?= getSortIconLaporan('pabrik', $sortBy ?? 'namabarang', $sortOrder ?? 'ASC') ?>
                                    </a>
                                </th>
                                <th>
                                    <a href="<?= getSortUrlLaporan('golongan', $sortBy ?? 'namabarang', $sortOrder ?? 'ASC', $search ?? '', $perPage ?? 50, $kodepabrik ?? '', $kodegolongan ?? '', $kondisiStok ?? 'semua') ?>" class="text-decoration-none text-dark d-inline-flex align-items-center gap-1">
                                        Golongan
                                        <?= getSortIconLaporan('golongan', $sortBy ?? 'namabarang', $sortOrder ?? 'ASC') ?>
                                    </a>
                                </th>
                                <th>Kandungan</th>
                            </tr>
